...
CResponse::CResponse(cec_opcode opcode) : m_opcode(opcode)
{
}
...
CCECBusDevice::CCECBusDevice(CCECProcessor *processor, cec_logical_address iLogicalAddress, uint16_t iPhysicalAddress /* = CEC_INVALID_PHYSICAL_ADDRESS */) : m_type(CEC_DEVICE_TYPE_RESERVED),
...
bool CCECBusDevice::SystemAudioModeRequest(void)
{
    ...
    return iPhysicalAddress != CEC_INVALID_PHYSICAL_ADDRESS && !!m_handler ? m_handler->TransmitSystemAudioModeRequest(m_iLogicalAddress, iPhysicalAddress) : false;
}
