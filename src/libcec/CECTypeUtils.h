...
public:
  ...
  /*!
   * @brief Get the ackmask for all devices of the same type as the given
   * logical address.
  ...
  static const char *ToString(const cec_opcode opcode) {
    ...
    case CEC_OPCODE_REPORT_SHORT_AUDIO_DESCRIPTORS:
      return "report short audio descriptors";
    default:
      return "UNKNOWN";
    }
  }
  ...
  static std::string ToString(const cec_command &command) {
    std::string dataStr;
    dataStr = StringUtils::Format(">> %1x%1x", command.initiator,
                                  command.destination);
    if (command.opcode_set == 1)
      dataStr += StringUtils::Format(":%02x", command.opcode);
    for (uint8_t iPtr = 0; iPtr < command.parameters.size; iPtr++)
      dataStr +=
          StringUtils::Format(":%02x", (unsigned int)command.parameters[iPtr]);
    return dataStr;
  }
};
} // namespace CEC
