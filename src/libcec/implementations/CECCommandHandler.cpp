...
CCECCommandHandler::CCECCommandHandler(
    CCECBusDevice *busDevice,
    int32_t iTransmitTimeout /* = CEC_DEFAULT_TRANSMIT_TIMEOUT */,
    int32_t iTransmitWait /* = CEC_DEFAULT_TRANSMIT_WAIT */,
    int8_t iTransmitRetries /* = CEC_DEFAULT_TRANSMIT_RETRIES */,
    int64_t iActiveSourcePending /* = 0 */)
    : m_busDevice(busDevice), m_processor(m_busDevice->GetProcessor()),
      m_iTransmitTimeout(iTransmitTimeout), m_iTransmitWait(iTransmitWait),
      m_iTransmitRetries(iTransmitRetries), m_bHandlerInited(false),
      m_bOPTSendDeckStatusUpdateOnActiveSource(false),
      m_vendorId(CEC_VENDOR_UNKNOWN),
      m_iActiveSourcePending(iActiveSourcePending), m_iPowerStatusRequested(0) {
}
...
bool CCECCommandHandler::HandleCommand(const cec_command &command) {
  ...
  case CEC_OPCODE_PLAY:
    // libCEC (currently) doesn't need to do anything with this, since player
    // applications handle it but it should not respond with a feature abort
    iHandled = COMMAND_HANDLED;
    break;
  case CEC_OPCODE_REQUEST_ARC_START:
    // Tv sends us request to enable ARC,
    // we respond with CEC_OPCODE_START_ARC request to TV
    // this is to test python ARC virtual sink e.g. for fake amps
    // actual response has to be sent from python client
    // LIB_CEC->AddLog(CEC_LOG_DEBUG, "Request for CEC_OPCODE_REQUEST_ARC_START:
    // %s", ToString(command).c_str());
    iHandled = HandleSystemAudioArcStart(command);
    break;
  case CEC_OPCODE_REPORT_ARC_STARTED:
    // After CEC_OPCODE_START_ARC request TV responds with
    // CEC_OPCODE_REPORT_ARC_STARTED
    iHandled = COMMAND_HANDLED;
    break;
  case CEC_OPCODE_REQUEST_SHORT_AUDIO_DESCRIPTORS:
    // TV is requesting Short Audio Descriptors after ARC has started
    {
      if (m_processor->CECInitialised() &&
          m_processor->IsHandledByLibCEC(command.destination)) {
        cec_command reply;
        cec_command::Format(reply, command.destination, command.initiator,
                            CEC_OPCODE_REPORT_SHORT_AUDIO_DESCRIPTORS);
        LIB_CEC->AddLog(CEC_LOG_DEBUG,
                        ">> reporting short audio descriptors (ack)");
        iHandled = Transmit(reply, false, true)
                       ? COMMAND_HANDLED
                       : CEC_ABORT_REASON_NOT_IN_CORRECT_MODE_TO_RESPOND;
      } else {
        iHandled = CEC_ABORT_REASON_NOT_IN_CORRECT_MODE_TO_RESPOND;
      }
    }
    break;
  default:
  ...
  if (iHandled == COMMAND_HANDLED)
    m_busDevice->SignalOpcode((command.opcode == CEC_OPCODE_FEATURE_ABORT &&
                               command.parameters.size > 0)
                                  ? (cec_opcode)command.parameters[0]
                                  : command.opcode);
  else
...
int CCECCommandHandler::HandleActiveSource(const cec_command &command) {
  if (command.parameters.size == 2) {
    uint16_t iAddress = ((uint16_t)command.parameters[0] << 8) |
...
int CCECCommandHandler::HandleDeckControl(const cec_command &command) {
  CCECPlaybackDevice *device =
      CCECBusDevice::AsPlaybackDevice(GetDevice(command.destination));
...
int CCECCommandHandler::HandleDeviceVendorCommandWithId(
  ...
  if (device && device->GetCurrentVendorId() == CEC_VENDOR_UNKNOWN &&
    ...
    if (HasSpecificHandler((cec_vendor_id)iVendorId)) {
      LIB_CEC->AddLog(CEC_LOG_TRAFFIC,
                      ">> process after replacing vendor handler: %s",
                      ToString(command).c_str());
      m_processor->OnCommandReceived(command);
      return COMMAND_HANDLED;
    }
  }

  if (iVendorId == CEC_VENDOR_PIONEER &&
...
int CCECCommandHandler::HandleFeatureAbort(const cec_command &command) {
  if (command.parameters.size == 2 &&
    ...
    m_processor->GetDevice(command.initiator)
        ->SetUnsupportedFeature((cec_opcode)command.parameters[0]);
  return COMMAND_HANDLED;
}
...
int CCECCommandHandler::HandleGiveAudioStatus(const cec_command &command) {
  if (m_processor->CECInitialised() &&
      m_processor->IsHandledByLibCEC(command.destination)) {
    CCECAudioSystem *device =
        CCECBusDevice::AsAudioSystem(GetDevice(command.destination));
    if (device) {
...
int CCECCommandHandler::HandleGiveDevicePowerStatus(
    const cec_command &command) {
  if (m_processor->CECInitialised() &&
      m_processor->IsHandledByLibCEC(command.destination)) {
    CCECBusDevice *device = GetDevice(command.destination);
...
int CCECCommandHandler::HandleGiveSystemAudioModeStatus(
    const cec_command &command) {
  if (m_processor->CECInitialised() &&
      m_processor->IsHandledByLibCEC(command.destination)) {
    CCECAudioSystem *device =
...
int CCECCommandHandler::HandleReportAudioStatus(const cec_command &command) {
  if (command.parameters.size == 1) {
    CCECAudioSystem *device =
        CCECBusDevice::AsAudioSystem(GetDevice(command.initiator));
    if (device) {
...
int CCECCommandHandler::HandleReportPhysicalAddress(
    const cec_command &command) {
  if (command.parameters.size == 3) {
    uint16_t iNewAddress = ((uint16_t)command.parameters[0] << 8) |
                           ((uint16_t)command.parameters[1]);
...
int CCECCommandHandler::HandleRequestActiveSource(const cec_command &command) {
  if (m_processor->CECInitialised()) {
    LIB_CEC->AddLog(CEC_LOG_DEBUG, ">> %i requests active source",
                    (uint8_t)command.initiator);
    m_processor->GetDevice(command.initiator)
        ->SetPowerStatus(CEC_POWER_STATUS_ON);

    std::vector<CCECBusDevice *> devices;
    for (size_t iDevicePtr = 0; iDevicePtr < GetMyDevices(devices);
         iDevicePtr++)
...
int CCECCommandHandler::HandleSetStreamPath(const cec_command &command) {
  ...
  if (command.parameters.size >= 2) {
    uint16_t iStreamAddress = ((uint16_t)command.parameters[0] << 8) |
                              ((uint16_t)command.parameters[1]);
    LIB_CEC->AddLog(
        CEC_LOG_DEBUG, ">> %s (%x) sets stream path to physical address %04x",
        ToString(command.initiator), command.initiator, iStreamAddress);
...
int CCECCommandHandler::HandleSystemAudioModeRequest(
    const cec_command &command) {
  if (m_processor->CECInitialised() &&
      m_processor->IsHandledByLibCEC(command.destination)) {
    CCECAudioSystem *device =
...
int CCECCommandHandler::HandleSystemAudioModeStatus(
    const cec_command &command) {
  if (command.parameters.size == 1) {
    CCECAudioSystem *device =
        CCECBusDevice::AsAudioSystem(GetDevice(command.initiator));
    if (device) {
      device->SetSystemAudioModeStatus(
          (cec_system_audio_status)command.parameters[0]);
      return COMMAND_HANDLED;
    }
  }
...
int CCECCommandHandler::HandleSystemAudioArcStart(const cec_command &command) {
  // LIB_CEC->AddLog(CEC_LOG_DEBUG, "HandleSystemAudioArcStart: %s",
  // ToString(command).c_str());
  if (command.parameters.size == 0) {
    LIB_CEC->AddLog(CEC_LOG_DEBUG,
                    "HandleSystemAudioArcStart: params ok, initiator %s",
                    ToString(command.initiator));

    CCECAudioSystem *device =
        CCECBusDevice::AsAudioSystem(GetDevice(command.destination));
    if (device) {
      LIB_CEC->AddLog(CEC_LOG_DEBUG,
                      "HandleSystemAudioArcStart: audio device %s ok",
                      ToString(command.destination));
  ...
  LIB_CEC->AddLog(
      CEC_LOG_DEBUG,
      "HandleSystemAudioArcStart: return CEC_ABORT_REASON_INVALID_OPERAND");
...
int CCECCommandHandler::HandleUserControlPressed(const cec_command &command) {
  ...
  if (command.parameters[0] == CEC_USER_CONTROL_CODE_POWER ||
    ...
    // CEC_USER_CONTROL_CODE_POWER and
    // CEC_USER_CONTROL_CODE_POWER_TOGGLE_FUNCTION operate as a toggle assume
    // CEC_USER_CONTROL_CODE_POWER_ON_FUNCTION does not
    if (command.parameters[0] == CEC_USER_CONTROL_CODE_POWER ||
        command.parameters[0] == CEC_USER_CONTROL_CODE_POWER_TOGGLE_FUNCTION) {
      cec_power_status status = device->GetCurrentPowerStatus();
      bPowerOn = !(status == CEC_POWER_STATUS_ON ||
    ...
    // we're not marked as active source, but the tv sends keypresses to us, so
    // assume it forgot to activate us
...
int CCECCommandHandler::HandleVendorCommand(
    const cec_command &UNUSED(command)) {
  return CEC_ABORT_REASON_INVALID_OPERAND;
}

int CCECCommandHandler::HandleVendorRemoteButtonDown(
    const cec_command &command) {
  if (command.parameters.size == 0)
    return CEC_ABORT_REASON_INVALID_OPERAND;

  LIB_CEC->AddLog(CEC_LOG_NOTICE,
                  "unhandled vendor remote button received with keycode %x",
...
void CCECCommandHandler::UnhandledCommand(const cec_command &command,
  ...
  if (m_processor->IsHandledByLibCEC(command.destination)) {
    LIB_CEC->AddLog(
        CEC_LOG_DEBUG, "sending abort with opcode %02x and reason '%s' to %s",
        command.opcode, ToString(reason), ToString(command.initiator));
    m_processor->TransmitAbort(command.destination, command.initiator,
...
size_t
CCECCommandHandler::GetMyDevices(std::vector<CCECBusDevice *> &devices) const {
  size_t iReturn(0);
...
CCECBusDevice *
CCECCommandHandler::GetDevice(cec_logical_address iLogicalAddress) const {
  return m_processor->GetDevice(iLogicalAddress);
}

CCECBusDevice *CCECCommandHandler::GetDeviceByPhysicalAddress(
    uint16_t iPhysicalAddress) const {
...
void CCECCommandHandler::SetPhysicalAddress(cec_logical_address iAddress,
                                            uint16_t iNewAddress) {
  if (!m_processor->IsHandledByLibCEC(iAddress)) {
    CCECBusDevice *otherDevice =
        m_processor->GetDeviceByPhysicalAddress(iNewAddress);
    CECClientPtr client =
        otherDevice ? otherDevice->GetClient() : CECClientPtr();

    CCECBusDevice *device = m_processor->GetDevice(iAddress);
    if (device)
      device->SetPhysicalAddress(iNewAddress);
    else
      LIB_CEC->AddLog(CEC_LOG_DEBUG, "device with logical address %X not found",
                      iAddress);
    ...
    if (client) {
      ...
      param.paramData = (void *)"Physical address in use by another device. "
    ...
    LIB_CEC->AddLog(CEC_LOG_DEBUG,
                    "ignore physical address report for device %s (%X) because "
                    "it's marked as handled by libCEC",
                    ToString(iAddress), iAddress);
  }
}

bool CCECCommandHandler::PowerOn(const cec_logical_address iInitiator,
                                 const cec_logical_address iDestination) {
  if (iDestination == CECDEVICE_TV)
    return TransmitImageViewOn(iInitiator, iDestination);

  return TransmitKeypress(iInitiator, iDestination,
                          CEC_USER_CONTROL_CODE_POWER) &&
         TransmitKeyRelease(iInitiator, iDestination);
}

bool CCECCommandHandler::TransmitImageViewOn(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination) {
  cec_command command;
...
bool CCECCommandHandler::TransmitStandby(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination) {
  cec_command command;
...
bool CCECCommandHandler::TransmitArcStarted(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination) {
  // https://zhuanlan.zhihu.com/p/80772947
  // Then get the CEC version, and then reply... followed by various requests.
  // 0xc3 (Request ARC initiation) will be sent when the various information of
  // the TV bar is collected At this time, the speaker sends 0XC0 (initiate arc)
  // to the TV Then the TV informs everyone 0x82 (active source) Then set 0x72
  // (SetSystemAudioMode) TV sent to speaker 0xc1 (report arc initiated) After
  // my TV is initialized, it will always send 0x82 to 0x0f, and then I must
  // reply 0xc0 or it will drop.
...
bool CCECCommandHandler::TransmitRequestActiveSource(
    const cec_logical_address iInitiator, bool bWaitForResponse /* = true */) {
  cec_command command;
  cec_command::Format(command, iInitiator, CECDEVICE_BROADCAST,
                      CEC_OPCODE_REQUEST_ACTIVE_SOURCE);

  return Transmit(command, !bWaitForResponse, false);
}

bool CCECCommandHandler::TransmitRequestCecVersion(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination,
...
bool CCECCommandHandler::TransmitRequestOSDName(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination,
    bool bWaitForResponse /* = true */) {
  cec_command command;
...
bool CCECCommandHandler::TransmitRequestAudioStatus(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination,
    bool bWaitForResponse /* = true */) {
  cec_command command;
...
bool CCECCommandHandler::TransmitRequestPhysicalAddress(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination,
    bool bWaitForResponse /* = true */) {
  cec_command command;
...
bool CCECCommandHandler::TransmitRequestPowerStatus(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, bool bUpdate,
    bool bWaitForResponse /* = true */) {
  if (iDestination == CECDEVICE_TV) {
  ...
  LIB_CEC->AddLog(CEC_LOG_DEBUG, "<< requesting power status of '%s' (%X)",
                  m_busDevice->GetLogicalAddressName(), iDestination);
...
bool CCECCommandHandler::TransmitRequestVendorId(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination,
    bool bWaitForResponse /* = true */) {
  cec_command command;
...
bool CCECCommandHandler::TransmitActiveSource(
    const cec_logical_address iInitiator, uint16_t iPhysicalAddress,
    bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, CECDEVICE_BROADCAST,
...
bool CCECCommandHandler::TransmitCECVersion(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, cec_version cecVersion,
    bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, iDestination,
...
bool CCECCommandHandler::TransmitInactiveSource(
    const cec_logical_address iInitiator, uint16_t iPhysicalAddress) {
  cec_command command;
  cec_command::Format(command, iInitiator, CECDEVICE_TV,
...
bool CCECCommandHandler::TransmitMenuState(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, cec_menu_state menuState,
    bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, iDestination,
...
bool CCECCommandHandler::TransmitOSDName(const cec_logical_address iInitiator,
                                         const cec_logical_address iDestination,
...
bool CCECCommandHandler::TransmitOSDString(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, cec_display_control duration,
    const char *strMessage, bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, iDestination,
...
bool CCECCommandHandler::TransmitPhysicalAddress(
    const cec_logical_address iInitiator, uint16_t iPhysicalAddress,
    cec_device_type type, bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, CECDEVICE_BROADCAST,
...
bool CCECCommandHandler::TransmitSetMenuLanguage(
    const cec_logical_address iInitiator, const char lang[4], bool bIsReply) {
  cec_command command;
  command.Format(command, iInitiator, CECDEVICE_BROADCAST,
...
bool CCECCommandHandler::TransmitPoll(const cec_logical_address iInitiator,
                                      const cec_logical_address iDestination,
                                      bool bIsReply) {
  cec_command command;
...
bool CCECCommandHandler::TransmitPowerState(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, cec_power_status state,
    bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, iDestination,
...
bool CCECCommandHandler::TransmitVendorID(
    const cec_logical_address iInitiator,
    const cec_logical_address UNUSED(iDestination), uint64_t iVendorId,
    bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, CECDEVICE_BROADCAST,
...
bool CCECCommandHandler::TransmitAudioStatus(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, uint8_t state, bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, iDestination,
...
bool CCECCommandHandler::TransmitSetSystemAudioMode(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, cec_system_audio_status state,
    bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, iDestination,
...
bool CCECCommandHandler::TransmitSetStreamPath(uint16_t iStreamPath,
  ...
  cec_command command;
  cec_command::Format(command, m_busDevice->GetLogicalAddress(),
                      CECDEVICE_BROADCAST, CEC_OPCODE_SET_STREAM_PATH);
...
bool CCECCommandHandler::TransmitSystemAudioModeStatus(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, cec_system_audio_status state,
    bool bIsReply) {
  cec_command command;
...
bool CCECCommandHandler::TransmitDeckStatus(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, cec_deck_info state,
    bool bIsReply) {
  cec_command command;
  cec_command::Format(command, iInitiator, iDestination,
...
bool CCECCommandHandler::TransmitKeypress(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, cec_user_control_code key,
    bool bWait /* = true */) {
  cec_command command;
  cec_command::Format(command, iInitiator, iDestination,
...
bool CCECCommandHandler::TransmitKeyRelease(
    const cec_logical_address iInitiator,
    const cec_logical_address iDestination, bool bWait /* = true */) {
  cec_command command;
  cec_command::Format(command, iInitiator, iDestination,
...
bool CCECCommandHandler::TransmitSystemAudioModeRequest(
    const cec_logical_address iInitiator, uint16_t iPhysicalAddress) {
  cec_command command;

  cec_command::Format(command, iInitiator, CECDEVICE_AUDIOSYSTEM,
...
bool CCECCommandHandler::Transmit(cec_command &command, bool bSuppressWait,
                                  bool bIsReply) {
  bool bReturn(false);
  ...
  // check whether the destination is not marked as not present or handled by
  // libCEC
  if (command.destination != CECDEVICE_BROADCAST && command.opcode_set) {
    CCECBusDevice *destinationDevice =
        m_processor->GetDevice(command.destination);
    cec_bus_device_status status = destinationDevice
                                       ? destinationDevice->GetStatus()
                                       : CEC_DEVICE_STATUS_NOT_PRESENT;
    if (status == CEC_DEVICE_STATUS_NOT_PRESENT) {
      LIB_CEC->AddLog(CEC_LOG_DEBUG,
                      "not sending command '%s': destination device '%s' "
                      "marked as not present",
                      ToString(command.opcode), ToString(command.destination));
      return bReturn;
    } else if (status == CEC_DEVICE_STATUS_HANDLED_BY_LIBCEC) {
      LIB_CEC->AddLog(CEC_LOG_DEBUG,
                      "not sending command '%s': destination device '%s' "
                      "marked as handled by libCEC",
                      ToString(command.opcode), ToString(command.destination));
      return bReturn;
    ...
    while (!bReturn && ++iTries <= iMaxTries) {
      if ((bReturn = m_processor->Transmit(command, bIsReply)) == true) {
        ...
        if (bExpectResponse) {
          ...
          LIB_CEC->AddLog(CEC_LOG_DEBUG,
                          bReturn ? "expected response received (%X: %s)"
                                  : "expected response not received (%X: %s)",
                          (int)expectedResponse, ToString(expectedResponse));
        }
      }
    }
  }
...
bool CCECCommandHandler::ActivateSource(
    bool bTransmitDelayedCommandsOnly /* = false */) {
  if (m_busDevice->IsActiveSource() && m_busDevice->IsHandledByLibCEC()) {
    {
    ...
    if (!bActiveSourceFailed && bSourceSwitchAllowed) {
      ...
      if (bTvPresent && !bActiveSourceFailed) {
        ...
        if (playbackDevice && SendDeckStatusUpdateOnActiveSource())
          bActiveSourceFailed =
              !playbackDevice->TransmitDeckStatus(CECDEVICE_TV, false);
      }
      ...
      if (bTvPresent && !bActiveSourceFailed) {
        ...
        if (audioDevice)
          bActiveSourceFailed =
              !audioDevice->TransmitSetSystemAudioMode(CECDEVICE_TV, false);
      }
    }
    ...
    if (bActiveSourceFailed || !bSourceSwitchAllowed) {
      LIB_CEC->AddLog(CEC_LOG_DEBUG,
                      "failed to make '%s' the active source. will retry later",
                      m_busDevice->GetLogicalAddressName());
      int64_t now(GetTimeMs());
      CLockObject lock(m_mutex);
      if (m_iActiveSourcePending == 0 || m_iActiveSourcePending < now)
        m_iActiveSourcePending =
            now + (int64_t)CEC_ACTIVE_SOURCE_SWITCH_RETRY_TIME_MS;
      return false;
    } else {
      CLockObject lock(m_mutex);
...
void CCECCommandHandler::RequestEmailFromCustomer(const cec_command &command) {
  ...
  std::map<cec_opcode, std::vector<cec_command>>::iterator it =
      m_logsRequested.find(command.opcode);
  if (it != m_logsRequested.end()) {
    for (std::vector<cec_command>::const_iterator it2 = it->second.begin();
  ...
  LIB_CEC->AddLog(
      CEC_LOG_NOTICE,
      "Unmapped code detected. Please send an email to support@pulse-eight.com "
      "with the following details, and if you pressed a key, tell us which one "
      "you pressed, and we'll add support for this it.\nCEC command: "
      "%s\nVendor ID: %s (%06x)",
      ToString(command).c_str(), ToString(m_vendorId), m_vendorId);
}
